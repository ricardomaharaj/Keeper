            ...
            <div className='container mx-auto'>
                <div className='col space-y-2 m-2 xl:mx-40'>
                    <div className='row space-x-2 p-2 justify-center'>
                        <Link to='/' className='text-xl'> KEEPER </Link>
                        <div className='p-1' onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>
...
function Home() {
    ...
    return <>
        ...
        <div className='grid123'>
            {notes
                ?.filter(x => x.title.includes(filter) || x.body.includes(filter))
                ?.sort((a, b) =>
                    sort === 'old'
                        ? (a.date > b.date) ? 1 : -1
                        : (a.date > b.date) ? -1 : 1
                ...
                ?.map((x, i) =>
                    <Link to={`/${x.id}`} className='bg1 bubble' key={i}>
                        ...
                        <div> {x.body.length > 200
                            ? x.body.substring(0, 197).padEnd(200, '.')
                            : x.body
                        } </div>
                        {x.date && <div className='subtext'> {new Date(x.date).toLocaleString()} </div>}
                    </Link>
                )}
        </div>
    </>
...
function NotePad() {
    ...
    let Delete = () => {
        NoteManager.deleteNote(parseFloat(id!)).then(() => { nav('/') })
    }
        ...
        <div className=''>
            {mode === Mode.View && <>
                <div className='col space-y-2'>
                    <div className='col bg1 bubble'>
                        ...
                        <div> {note?.body} </div>
                        <div className='subtext'> {new Date(note?.date!).toLocaleString()} </div>
                    </div>
                    <div className='row space-x-2'>
                        <button className='red bubble' onClick={() => setMode(Mode.Delete)}> Delete </button>
                        <button className='ylw bubble' onClick={() => setMode(Mode.Edit)}> Edit </button>
                        <button className='blu bubble' onClick={() => nav('/')}> Home </button>
                    </div>
                </div>
            </>}
            {mode === Mode.Edit && <>
                <div className='col space-y-2'>
                    <div className='col space-y-2'>
                        <input placeholder='Title' id='title' type='text' className='bubble bg1' defaultValue={note?.title} />
                        <textarea rows={10} id='body' className='bubble bg1' defaultValue={note?.body} />
                        <div className='subtext'> {new Date(note?.date!).toLocaleString()} </div>
                    </div>
                    <div className='row space-x-2'>
                        <button className='grn bubble' onClick={Save}> Save </button>
                        <button className='ylw bubble' onClick={() => setMode(Mode.View)}> Cancel </button>
                        <button className='red bubble' onClick={() => setMode(Mode.Delete)}> Delete </button>
                    </div>
                </div>
            </>}
            {mode === Mode.Delete && <>
                <div className='col space-y-2'>
                    ...
                    <div className='row space-x-2'>
                        <button className='red bubble' onClick={Delete}> Confirm Delete </button>
                        <button className='ylw bubble' onClick={() => setMode(Mode.Edit)}> Edit </button>
                        <button className='blu bubble' onClick={() => setMode(Mode.View)}> Cancel </button>
                    </div>
                </div>
            </>}
        </div>
